  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Require service-role bearer token. This route is only meant for
        // internal/privileged callers; the app itself invokes
        // `sendTransactionalEmailServer` directly and does not hit this route.
        ...
        for (let i = 0; i < len; i++) ok = ((a[i] ?? 0) === (b[i] ?? 0)) && ok
        if (!ok) {
          return Response.json({ error: 'Unauthorized' }, { status: 401 })
        }
        ...
        const { error: enqueueError } = await supabase.rpc('enqueue_email', {
          ...
          payload: {
            ...
            from: `${SITE_NAME} Köln <buchung@${FROM_DOMAIN}>`,
